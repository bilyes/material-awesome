...
local config = require('widget.stock.config')

local function show_warning(message, title)
    if title == nil then
...
local symbol = config.symbol or 'VFV'
local timeout = config.timeout or 1800

local fetch_quotes_cmd = filesystem.get_configuration_dir() .. "widget/stock/fetch-quotes.sh " .. symbol
...
--local current_stock_widget = wibox.widget {
    --{
        --{
            --{
                --id = 'icon',
...
local function update_widget(widget, stdout, stderr)
    --if stderr ~= '' then
        --if not warning_shown then
            ...
            --show_warning(stderr, 'ok it has to be right around here man come the fuck on already!!')
            --show_warning(stout, 'here is the out')
        --end
        --return
    --end
    ...
    local price_string = string.format('%.2f', result.data.getQuoteBySymbol.price)
    --show_warning(price_srting, 'this is what we got')
    widget:set_text(price_string)

    --current_stock_widget:update(result.current)
...
--stock_widget:buttons(awful.util.table.join(awful.button({}, 1, function()
...
watch(
    --string.format(stock_one_cal_api, payload),
    fetch_quotes_cmd,
